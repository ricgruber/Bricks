...
class Autoblog_Render_Feeds_Form extends Autoblog_Render {
	...
	*/
	private function _render_form() {
		?><div class="postbox autoblogeditbox" id="ab-<?php echo esc_attr( $this->feed_id ) ?>">
			...
			<h3 class="hndle"><span><?php esc_html_e( 'Feed', 'autoblogtext' ) ?>:  <?php echo esc_html( stripslashes( $this->title ) ) ?></span></h3>
			...
			<div class="inside">
				...
				<div class="tablenav">
					<div class="alignright">
						<a class="button" href='admin.php?page=autoblog_admin'><?php esc_html_e( 'Cancel', 'autoblogtext' ) ?></a>
						&nbsp;&nbsp;&nbsp;
						<input class="button-primary delete save" type="submit" value="<?php echo !empty( $this->feed_id ) ? esc_attr__( 'Update feed', 'autoblogtext' ) : esc_attr__( 'Create feed', 'autoblogtext' ) ?>" style="margin-right: 10px;">
					</div>
				</div>
			</div>
		</div><?php
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[postdate]" class="field">
					<option value="current"<?php selected( 'current', $this->postdate ) ?>><?php esc_html_e( 'Imported date', 'autoblogtext' ) ?></option>
					<option value="existing"<?php selected( 'existing', $this->postdate ) ?>><?php esc_html_e( 'Original posts date', 'autoblogtext' ) ?></option>
				</select>
	...
	*/
	private function _render_form_author_section() {
		...
		<tr>
			<td valign="top" class="heading"><?php esc_html_e( 'If author in feed does not exist locally use', 'autoblogtext' ) ?></td>
			<td valign="top">
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[useexcerpt]" class="field">
					...
					<option value="2"<?php selected( 2, $this->useexcerpt ) ?>><?php esc_html_e( 'Use Excerpt', 'autoblogtext' ) ?></option>
				</select>
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[processfeed]" class="field">
					<option value="0"<?php selected( 0, $this->processfeed ) ?>><?php esc_html_e( 'Never (paused)', 'autoblogtext' ) ?></option>

					<option value="5"<?php selected( 5, $this->processfeed ) ?>><?php esc_html_e( 'every 5 minutes', 'autoblogtext' ) ?></option>
					<option value="10"<?php selected( 10, $this->processfeed ) ?>><?php esc_html_e( 'every 10 minutes', 'autoblogtext' ) ?></option>
					<option value="15"<?php selected( 15, $this->processfeed ) ?>><?php esc_html_e( 'every 15 minutes', 'autoblogtext' ) ?></option>
					<option value="20"<?php selected( 20, $this->processfeed ) ?>><?php esc_html_e( 'every 20 minutes', 'autoblogtext' ) ?></option>
					<option value="25"<?php selected( 25, $this->processfeed ) ?>><?php esc_html_e( 'every 25 minutes', 'autoblogtext' ) ?></option>

					<option value="30"<?php selected( 30, $this->processfeed ) ?>><?php esc_html_e( 'every 30 minutes', 'autoblogtext' ) ?></option>
					<option value="60"<?php selected( 60, $this->processfeed ) ?>><?php esc_html_e( 'every hour', 'autoblogtext' ) ?></option>
					<option value="90"<?php selected( 90, $this->processfeed ) ?>><?php esc_html_e( 'every 1 hour 30 minutes', 'autoblogtext' ) ?></option>
					<option value="120"<?php selected( 120, $this->processfeed ) ?>><?php esc_html_e( 'every 2 hours', 'autoblogtext' ) ?></option>
					<option value="150"<?php selected( 150, $this->processfeed ) ?>><?php esc_html_e( 'every 2 hours 30 minutes', 'autoblogtext' ) ?></option>
					<option value="300"<?php selected( 300, $this->processfeed ) ?>><?php esc_html_e( 'every 5 hours', 'autoblogtext' ) ?></option>
					<option value="1449"<?php selected( 1449, $this->processfeed ) ?>><?php esc_html_e( 'every day', 'autoblogtext' ) ?></option>
				</select>
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[startfromday]" class="field">
					<option></option>
					<?php for ( $n = 1; $n <= 31; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->startfrom ) && $n == date( 'j', $this->startfrom ) ) ?>><?php echo $n ?></option>
					<?php endfor; ?>
				</select>
				<select name="abtble[startfrommonth]" class="field">
					<option></option>
					<?php for ( $n = 1; $n <= 12; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->startfrom ) && $n == date( 'n', $this->startfrom ) ) ?>><?php echo date( 'M', strtotime( date( 'Y-' . $n . '-1' ) ) ) ?></option>
					<?php endfor; ?>
				</select>
				<select name="abtble[startfromyear]" class="field">
					<option></option>
					<?php for ( $n = date( "Y" ) - 1; $n <= date( "Y" ) + 9; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->startfrom ) && $n == date( 'Y', $this->startfrom ) ) ?>><?php echo $n ?></option>
					<?php endfor; ?>
				</select>
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[endonday]" class="field">
					<option></option>
					<?php for ( $n = 1; $n <= 31; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->endon ) && $n == date( 'j', $this->endon ) ) ?>><?php echo $n ?></option>
					<?php endfor; ?>
				</select>
				<select name="abtble[endonmonth]" class="field">
					<option></option>
					<?php for ( $n = 1; $n <= 12; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->endon ) && $n == date( 'n', $this->endon ) ) ?>><?php echo date( 'M', strtotime( date( 'Y-' . $n . '-1' ) ) ) ?></option>
					<?php endfor; ?>
				</select>
				<select name="abtble[endonyear]" class="field">
					<option></option>
					<?php for ( $n = date( "Y" ) - 1; $n <= date( "Y" ) + 9; $n++ ) : ?>
					<option value="<?php echo $n ?>"<?php selected( !empty( $this->endon ) && $n == date( 'Y', $this->endon ) ) ?>><?php echo $n ?></option>
					<?php endfor; ?>
				</select>
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[forcessl]" class="field">
					...
					<option value="no"<?php selected( 'no', $this->forcessl ) ?>><?php esc_html_e( 'No', 'autoblogtext' ) ?></option>
				</select>
		...
		<tr>
			...
			<td valign="top">
				<select name="abtble[overridedups]" class="field">
					...
					<option value="yes"<?php selected( 'yes', $this->overridedups ) ?>><?php esc_html_e( 'Yes', 'autoblogtext' ) ?></option>
				</select>
